...
module "fnc-inventory-on-cdc-payment-created" {
  source = "../../resources/lambda"

  layers        = [aws_lambda_layer_version.inventory_common_layer.arn]
  env_prefix    = terraform.workspace
  function_name = "inventory-on-cdc-payment-created"
  source_dir    = "../services/inventory/dist/on-cdc-payment-created"
  output_path   = "../archived/inventory/on-cdc-payment-created.zip"
  depends_on = [
    aws_lambda_layer_version.inventory_common_layer
  ]
  env = {
  ...
  policies_arn       = [var.connect_rds_policy_arn]
  timeout            = 3
  q_arn              = var.queue_arn_mapper["${terraform.workspace}-inventory-cdc-payment-created"]
}
